...
flags.DEFINE_integer('batch_size', 1000, 'minibatch size.')
...
def predict_alldata(sess, model, minibatch_iter, size):
    ...
    finished = False
    while not finished:
        feed_dict_val, batch_labels, finished, _  = minibatch_iter.incremental_node_val_feed_dict(size, iter_num, test=True)
        node_outs_val = sess.run([model.preds, model.loss], 
                         feed_dict=feed_dict_val)
...
def train(train_data, test_data=None):
   ...
   # features = train_data[1]
    ...
    # Load trained model
    if os.path.exists("KITE_supervisedTrainedModel_MC_marginal/"):
    ...
    for var in tf.trainable_variables():
    	var_to_save.append(var)
    saver = tf.train.Saver(var_to_save)
    save_path = saver.save(sess, "KITE_supervisedTrainedModel_MC_marginal/model.ckpt")
    ...
    #
    # val_cost, duration = incremental_evaluate(sess, model, minibatch, FLAGS.batch_size, test=True)
    
    # with open(log_dir() + "test_stats_train.txt", "w") as fp:
    #     fp.write("loss={:.5f} ".
    #             format(val_cost))
