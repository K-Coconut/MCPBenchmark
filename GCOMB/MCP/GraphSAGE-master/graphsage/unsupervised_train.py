...
def incremental_evaluate(sess, model, minibatch_iter, size):
    ...
    while not finished:
        ...
        iter_num += 1
        outs_val = sess.run([model.loss, model.ranks, model.mrr], feed_dict=feed_dict_val)
        val_losses.append(outs_val[0])
        val_mrrs.append(outs_val[2])
    return np.mean(val_losses), np.mean(val_mrrs), (time.time() - t_test)

def save_val_embeddings(sess, model, minibatch_iter, size, out_dir,id, mod=""):
    val_embeddings = []
    finished = False
    seen = set([])
    nodes = []
...
def train(train_data, test_data=None):
        ...
        while not minibatch.end():
            ...
            if total_steps % FLAGS.print_every == 0:
                print("Iter:", '%04d' % iter, 
                      ...
                      "train_mrr=", "{:.8f}".format(train_mrr),
                      "train_mrr_ema=", "{:.8f}".format(train_shadow_mrr), # exponential moving average
                      "val_loss=", "{:.8f}".format(val_cost),
                      "val_mrr=", "{:.8f}".format(val_mrr),
    ...
    if FLAGS.save_embeddings:
        ...
        save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(),0)
        save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(),1)
