...
class NodeMinibatchIterator(object):
   ...
   #     deg=np.zeros((len(self.id2idx),))
      ...
      #  list_top_nodes_and_sampled_neighbors=list()#np.array([])#set()
        nbr_processed={}
       # np_array_neighbors = {}
        print(sorted([self.G.degree(i) for i in top_degree_nodes]))
   ...
   #     set_top_nodes_and_sampled_neighbors =set(list_top_nodes_and_sampled_neighbors)
      ...
      #  print(" type ", set(list_of_all_neighbors))
        ...
        for node in dict_map_real_to_counter_node.keys():
            neighbors = list(set([dict_map_real_to_counter_node[e] for e in self.G.neighbors(node) if e in dict_map_real_to_counter_node]).intersection(set_top_nodes_and_sampled_neighbors))
            samples = np.random.choice(neighbors, self.max_degree, replace=len(neighbors) < self.max_degree)
            adj_counter[dict_map_real_to_counter_node[node]] = samples
