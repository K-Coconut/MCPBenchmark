...
def getnewgraph(graph_dir, num_k, neighborhood_sampling):
	...
	graph_filename=graph_dir + "-edges.txt"
	main_graph = nx.read_edgelist(graph_filename, nodetype=int)
	m=len(main_graph)
	...
	for k, v in embeddings_dict.items():
		...
		#embeddings_dict[k][0]=main_graph.degree(k)
	...
	time_end_normalize = time.time()
	time_normalize = time_end_normalize - time_begin_normalize
	print(" normalize time ", time_normalize)
...
class Graph:
	def __init__(self,graph_dir, num_k, nbs):
		#solve(graph_dir)
		#exit()
		(self.graphx, unscaled_embedding, self.numNodes, self.top_tenpct_nodes, self.dict_node_sampled_neighbors) = getnewgraph(graph_dir, num_k, nbs)
