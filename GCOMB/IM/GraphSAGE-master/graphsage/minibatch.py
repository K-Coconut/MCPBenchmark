...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
            ...
            batch_size=100, max_degree=25, mode="None",prefix =None, budget=None, bud_mul_fac=None, sampling_freq=None,
            **kwargs):
        ...
        #	self.val_nodes=[n for n in self.G.nodes() if self.deg[id2idx[n]] > 0][
        #				   int(len(G) / 2) - int(0.1 * len(G) / 2):int(len(G) / 2)]

        #	self.no_train_nodes_set=set(self.val_nodes + self.test_nodes)
        #	self.train_nodes=set(G.nodes()).difference(self.no_train_nodes_set)
            # don't train on nodes that only have edges to test set
        #	self.train_nodes=[n for n in self.train_nodes if self.deg[id2idx[n]] > 0][
        #					 0:int(len(G)) - int(0.1 * len(G) / 2)]
            print(" all nodes ", len(self.all_nodes))
            self.train_nodes = [x for x in self.all_nodes if float(self.label_map[x][0])>0]


            print(" non zero train nodes" , len(self.train_nodes))
            self.features=self.scale_features(np.load(os.path.join(prefix, "large_graph-feats.npy")))
            print("prefix", prefix)
            ...
            self.adj, self.deg, self.features, self.top_degree_nodes, time_sampling_feature, self.dict_map_couter_to_real_node,sampled_nbrs_for_rl=self.construct_adj_and_feat_for_top_degree_testing(
                budget, bud_mul_fac, sampling_freq, prefix)
            ...
            with open(prefix + '-sampled_nbrs_for_rl' + '.pickle'+"_"+str(budget)+"_nbs_"+str(sampling_freq), 'wb') as handle:
    ...
    def construct_adj(self):
        #percentage_of_neighbors_samples = 0.1
        ...
        for nodeid in self.G.nodes():
            ...
            if len(neighbors) == 0:
                c += 1
                # adj[self.id2idx[nodeid], :] = nodeid * np.ones(self.max_degree)
                # edge weight adj, set 0 for each edge weight for isolated nodes
                adj[self.id2idx[nodeid], :] = np.zeros(self.max_degree)
                continue

        #	print(nodeid, neighbors, self.G[nodeid])
            ...
            #	print(neighbors, neighbors_weights)
                neighbors = np.random.choice(neighbors,  self.max_degree,replace=True)

            #  replace edge weight adj as adj
            adj[self.id2idx[nodeid], :] = [self.G[nodeid][v]['weight'] for v in neighbors]
        print("missing: ", c)
        return adj, deg
    ...
    def construct_adj_and_feat_for_top_degree_testing(self, budget=None, bud_mul_fac=None, sampling_freq=None, prefix=None):
        ...
        degree_of_nodes_all=self.G.out_degree(weight='weight')
        ...
        total_top_degree_nodes = -1 * min(int(bud_mul_fac + 50), len(self.G))
        top_degree_nodes=set(np.argpartition(degree_of_nodes_all, total_top_degree_nodes)[total_top_degree_nodes:])
        print(" top deg ", len(top_degree_nodes))

        adj={}
        deg=np.zeros((len(self.id2idx),))

        set_top_nodes_and_sampled_neighbors=set()
        nbr_processed={}
        print(" time a" , time.time() - time_sampling_beg)
        for nodeid in top_degree_nodes:
            ...
            set_top_nodes_and_sampled_neighbors.add(dict_map_real_to_counter_node[nodeid])
            ...
            for each_nbr in neighbors:
                    ...
                    # replace edge weight adj as adj
                    current_nbrs_counter_node.append(self.G[nodeid][each_nbr]['weight'])

            adj[dict_map_real_to_counter_node[nodeid]] = np.asarray(current_nbrs_counter_node)
        ...
        list_of_all_neighbors = np.array(list(set(list_of_all_neighbors)))
        print(" len list of all neighbors", len(list_of_all_neighbors))
        print("sampling freq" , sampling_freq)
        number_of_neighbors_reqd  = len(list_of_all_neighbors)
        #CHANGED
        number_of_neighbors_reqd  = int(float(sampling_freq) *len(list_of_all_neighbors))
        degree_out_of_all_neighbors = np.array([ degree_of_nodes_all[x] for x in list_of_all_neighbors])
        useful_neighbors  = {}
        weight_sum_for_neighbor = {}

        in_degree_neighbors={}

        #indegree_weights_for_nbrs=degree_in_of_all_neighbors / sum(degree_in_of_all_neighbors)
        outdegree_weights_for_nbrs=degree_out_of_all_neighbors / sum(degree_out_of_all_neighbors)

        neighbors_sampled= list_of_all_neighbors
        #CHANGED
        neighbors_sampled =np.random.choice(list_of_all_neighbors, number_of_neighbors_reqd, p=outdegree_weights_for_nbrs,
                                              replace=False)
        print("len of sampled neighbors ", len(neighbors_sampled))


        for node in set_top_nodes_and_sampled_neighbors:
            norm_value_out=degree_of_nodes_all[dict_map_couter_to_real_node[node]] * 1.0 / sum_weight_out

            features[node][0]=norm_value_out

            if(dict_map_couter_to_real_node[node] in top_degree_nodes):

                neighbors=set(np.array(self.G.neighbors(dict_map_couter_to_real_node[node])))

                sampled_nbrs_for_rl[dict_map_couter_to_real_node[node]] = neighbors.intersection(neighbors_sampled)
               # print(dict_map_couter_to_real_node[node], len(sampled_nbrs_for_rl[dict_map_couter_to_real_node[node]]))

       # adj = adj[0:len(set_top_nodes_and_sampled_neighbors) +1]
        adj_counter = len(set_top_nodes_and_sampled_neighbors) * np.zeros((len(set_top_nodes_and_sampled_neighbors) + 1, self.max_degree))

        for key in range(adj_counter.shape[0]):
            adj_counter[key] = adj.get(key, np.asarray([0 for _ in range(self.max_degree)]))
        adj = adj_counter
        features = features[0:len(set_top_nodes_and_sampled_neighbors) + 1]
      ...
      #  print(adj)
        ...
        #
        print(" len sampled nbrs for rl", len(sampled_nbrs_for_rl))
