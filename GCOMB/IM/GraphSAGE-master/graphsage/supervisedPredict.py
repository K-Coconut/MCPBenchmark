...
flags.DEFINE_integer('num_k', 0, 'Budget')
flags.DEFINE_float('neighborhood_sampling', 0.005, 'neighboor hood for RL')
flags.DEFINE_float('bud_mul_fac', 300, 'bud mulp factor, 3 for WC, 10 for IC and TV')
...
def incremental_predict(sess, model, minibatch_iter, size):
    ...
    while not finished:
        feed_dict_val, batch_labels, finished, _ = minibatch_iter.incremental_node_testing_feed_dict_nodes_top_degree(
            size, iter_num)
        node_outs_val = sess.run([model.node_temp_embeddings, model.preds, model.loss],
                                 feed_dict=feed_dict_val)
...
def predict(train_data, test_data=None):
    ...
    minibatch = NodeMinibatchIterator(G,
                                      ...
                                      budget=num_k, bud_mul_fac=FLAGS.bud_mul_fac, mode="test",
    ...
    for index in range(0, len(pred_classes)):
        active_one_prob_dict[minibatch.dict_map_couter_to_real_node[minibatch.top_degree_nodes[index]]] = \
        predict_prob[index][0]

    #	bottom_nodes, top_nodes = bipartite.sets(G)
    # bottom_nodes = list(bottom_nodes)
    total_top_ten_percent = len(active_one_prob)  # int(num_k*FLAGS.bud_mul_fac)#int(0.25*len(bottom_nodes))
    ...
    result_top_percent = FLAGS.train_prefix + "_top_ten_percent.txt" + "_" + str(num_k) + "_nbs_" + str(FLAGS.neighborhood_sampling)
    ...
    dict_node_scores_file_name = FLAGS.train_prefix + "_node_scores_supgs" + "_" + str(num_k) + "_nbs_" + str(FLAGS.neighborhood_sampling)
    import pickle
    ...
    result_top_percent = FLAGS.train_prefix + "_top_ten_percent_analyse.txt" + "_" + str(num_k) + "_nbs_" + str(
    ...
    embedding_file_name = FLAGS.train_prefix + "_embeddings.npy" + "_" + str(num_k) + "_nbs_" + str(FLAGS.neighborhood_sampling)
    ...
    time_rl_prep_file_name = FLAGS.train_prefix + "_num_k_" + str(
        FLAGS.num_k) + "_time.txt" + "_" + str(num_k) + "_nbs_" + str(FLAGS.neighborhood_sampling)
...
def main(argv=None):
    ...
    tracemalloc.stop()
    f = open(os.path.join(FLAGS.train_prefix, "memory_gnn_budget_%d.txt" % FLAGS.num_k), 'w')
