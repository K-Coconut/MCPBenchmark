...
def gen_setcover_inst(total_nodes,fract):
    ...
    for i in range(num_dual):
        ...
        g.add_edge(k1, i + num_primal, weight=weight)
        weight = sample(weight_list, 1)[0]
        k2 = np.random.randint(num_primal)
        g.add_edge(k2, i + num_primal, weight=weight)
        for j in range(num_primal):
            ...
            if r < p:
                weight = sample(weight_list, 1)[0]
                g.add_edge(j, i + num_primal, weight=weight)
                colHasOneBool[j] = 1
    ...
    for j in range(num_primal):
        if colHasOneBool[j] == 0:
            ...
            weight = sample(weight_list, 1)[0]
            g.add_edge(j, randrow + num_primal, weight=weight)
