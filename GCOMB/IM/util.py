...
#	embeddings_ordered = np.load(embedding_file)
	...
	with open(embedding_file, 'rb') as handle:
		embeddings_dict = pickle.load(handle, encoding='bytes')

	#embeddings_ordered = np.append(empty_column_for_cover, embeddings_ordered, axis=1)

	main_graph = nx.read_edgelist(graph_dir + "edges.txt", nodetype=int, data=(('weight', float), ), create_using=nx.DiGraph())
	m = len(main_graph)
...
#	dict_node_scores_file_name=graph_dir + "_node_scores_supgs_{}.pickle".format(budget_used)
	with open(dict_node_scores_file_name, 'rb') as handle:
		dict_sup_gs_scores =pickle.load(handle, encoding='bytes')
...
#	dict_node_sampled_neighbors_file_name=graph_dir + "-sampled_nbrs_for_rl.pickle_{}".format(budget_used)
	with open(dict_node_sampled_neighbors_file_name, 'rb') as handle:
		dict_node_sampled_neighbors =pickle.load(handle, encoding='bytes')
...
class Graph:
	def __init__(self, dimEmbedding, episodeNum, num_k, dataset, weight_model):
		cons_to_add = 5000
		self.graph_dir = "./GraphSAGE-master/real_data/%s/%s/train/" % (dataset, weight_model)
...
def init(dataset, learningRate, numOfEpochs, batchSize, dimension, weight_model):
	nn.init(dataset, learningRate, numOfEpochs, batchSize, dimension, weight_model)
...
# influence: Number
def getShortReward(nodeSelected, graphid, previous_spread, weight_model):
	# additionCount = 0
	...
	if(len(graphEnv.graphEnvironment[graphid].state)==0):
		print(" first step ")
		shortReward = calculate_spread_mc_sim(graphEnv.graphEnvironment[graphid].graph_dir, [nodeSelected],weight_model)
...
#	print("before ",initial_spread, seed_nodes_earlier, new_node_to_be_added)
	...
	final_spread  = calculate_spread_mc_sim(graphEnv.graphEnvironment[graphid].graph_dir, seed_nodes_later,weight_model)
