...
def init(dataset, learningRate1, numOfEpochs1, batchSize1, dimension1, weight_model):
	global learningRate, batchSize, dimension, numOfEpochs, global_step, model, model_dir_name
	...
	if model.latest_checkpoint():
		global_step = model.get_variable_value('global_step')
...
def trainNeuralNet(historyOfTuples):
	...
	#	print(" y train ",y_train)
		...
		input_fn = tf.estimator.inputs.numpy_input_fn(x={'mu_selected': mu_s, 'mu_left': mu_l, 'mu_v': mu_v}, y=y_train, batch_size=3, num_epochs=50, shuffle=False)
...
def getBestNode(graphid, stateIdx):
	...
	for node in vertices:
		#print("node ", node)
		qValue = float(predictions.__next__())
