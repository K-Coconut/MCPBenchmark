...
double MvcEnv::calculateCoverage(int* sol)
{
    double numCoveredNodes = 0;
    std::set<int> coveredNodes_set;
    int p = 0;
    for (int i = 1; i <= sol[0]; i++)
    {
        for (auto &neigh : graph->adj_list[sol[i]])
        {
            if (coveredNodes_set.count(neigh) == 0)
            {
                coveredNodes_set.insert(neigh);
                numCoveredNodes++;
            }
        }
    }
    return numCoveredNodes;
}