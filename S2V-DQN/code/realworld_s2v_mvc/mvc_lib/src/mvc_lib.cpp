...
int Init(const int argc, const char** argv)
{
    ...
    for (int i = 0; i < cfg::batch_size; ++i)
        // size set smaller than 2100
        list_pred[i] = new std::vector<double>(2100);
    return 0;
}
...
double Test(const int gid)
{
    ...
    {
        cost++;
        list_pred[0] = new std::vector<double>(int(test_env->graph->num_nodes));
...
double GetBudgetedSol(const int gid, int* sol, int budget)
{
    std::vector< std::shared_ptr<Graph> > g_list(1);
    std::vector< std::vector<int>* > states(1);
    ...
    sol[0] = len;
    cost = test_env->calculateCoverage(sol);
    return cost; 
}