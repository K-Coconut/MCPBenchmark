...
public:
	...
	/// Get out-file name
	static std::string get_out_file_name(const std::string graphName, const std::string algName, const std::string weightModel, const int seedsize,
		const std::string probDist, const float probEdge, const float eps, const int n_iter)
	{
		if (probDist == "UNI")
		{
			return graphName + "_" + algName + "_" + weightModel + "_k" + std::to_string(seedsize) + "_" + probDist + std::
				to_string(probEdge) + "_eps_" + std::to_string(eps) + "_n_iter_" + std::to_string(n_iter); 
		}
		return graphName + "_" + algName + "_" + weightModel + "_k" + std::to_string(seedsize) + "_" + probDist + "_eps_" + std::to_string(eps) + "_n_iter_" + std::to_string(n_iter);
	}
	...
	static void write_result(const std::string& outFileName, const TResult& resultObj, const std::string& outFolder)
	{
		...
		std::cout << "  |Memory usage: " << disp_mem_usage("")  << std::endl;
		std::cout << "   --------------------" << std::endl;
		...
		if (outFileNew.is_open())
		{
			...
			outFileNew << "Memory usage: " << disp_mem_usage("")  << std::endl;
			outFileNew.close();
		}
	}
