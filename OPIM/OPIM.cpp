...
int main(int argc, char* argv[])
{
	...
	const std::string infoldername = Arg._dir + Arg._graphname + "/IM/" + Arg._weightModel + "/";
	if (Arg._func == 0 || Arg._func == 2)
	{
		// Format the graph
		GraphBase::format_graph(infoldername, Arg._mode);
	...
	Graph graph = GraphBase::load_graph(infoldername, true, Arg._probDist, Arg._probEdge);
	if (Arg._model == LT)
	{
