...
public:
	...
	std::string _graphname = "BrightKite"; // Graph name. Default is "facebook".
	...
	std::string _weightModel = "WC"; // Algorithm. Default is oneHop.
	...
	int _n_iter = 0; // number of times to repeat experiments
	...
	Argument(int argc, char* argv[])
	{
		...
		for (int ind = 1; ind < argc; ind++)
		{
			...
			else if (!param.compare("-wmodel")) _weightModel = value;
			else if (!param.compare("-n_iter")) _n_iter = stoi(value);;
		}
		...
		_outFileName = TIO::get_out_file_name(_graphname, _algName + postfix, _weightModel , _seedsize, _probDist, _probEdge, _eps, _n_iter);
	...
	{
		return TIO::get_out_file_name(_graphname, algName, _weightModel, _seedsize, _probDist, _probEdge, _eps, _n_iter);
	}
};
