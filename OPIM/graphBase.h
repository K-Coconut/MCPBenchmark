#pragma once
#define ASSERT(v) {if (!(v)) {std::cerr<<"ASSERT FAIL @ "<<__FILE__<<":"<<__LINE__<<std::endl; exit(1);}}

class GraphBase
{
public:
	/// Format the input for future computing, which is much faster for loading. Vector serialization is used.
	static void format_graph(const std::string graph_folder, const std::string mode)
	{
		std::string filename = graph_folder + "edges.txt" ;
		std::string attribute = graph_folder + "attribute.txt";
		...
		float weight = 0.0;
		std::cout << graph_folder << std::endl;
		std::ifstream cin((graph_folder + "attribute.txt").c_str());
		ASSERT(!cin == false);
		std::string s;
		while (cin >> s)
		{
			if (s.substr(0, 2) == "n=")
			{
				numV = atoi(s.substr(2).c_str());
				continue;
			}
			if (s.substr(0, 2) == "m=")
			{
				numE = atoi(s.substr(2).c_str());
				continue;
			}
			ASSERT(false);
		}
	...
	static Graph load_graph(std::string infoldername, const bool isReverse, const std::string probDist = "load",
									const float probEdge = float(0.1))
	{
		std::string graphName = infoldername + "edges.txt";
		Graph graph;
