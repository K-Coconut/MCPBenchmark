...
class NetworkEnv(object):
    ...
    def __init__(self, fullGraph, seeds=[], max_T=100, influence_algo=influence, opt_reward=None, nop_r=0, times_mean=1,
                 bad_reward=0, normalize=False, clip_min=None, clip_max=None, weight_model="CONST"):
    ...
    def step(self, action):
        ...
        if action == -1:
            self.done = True
            self.reward_ = parallel_influence(self.graph, full_graph=self.fullGraph, influence=self.influence_algo,
                                              times=self.times_mean, weight_model=self.weight_model)
            if self.normalize:
