import pickle
from expts.gengraph import random_sbm, random_ssm
import os
import numpy as np
...
def gen_ssm(num=100,n=1000, c=10, p_b=0.01, std=30, output_file="rand_ssm"):
    l = [random_ssm(n, c, p_b, std, seed=1) for i in range(num)]
    with open(os.path.join('data', 'rt', f"{output_file}_{num}_sample.pkl"), 'wb') as f:
        pickle.dump(l,f)
