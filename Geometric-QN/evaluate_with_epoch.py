...
def arg_parse():
    ...
    parser.add_argument('--type', dest='network_type', type=str, default='rt')
    ...
    parser.add_argument('--save_model', dest='save_model', type=str, default='1_trainset_',
...
os.makedirs("results/%s" % args.dataset, exist_ok=True)


def load_checkpoint(model, model_prefix, epoch):
    model_path = '%s%d.pth' % (model_prefix, epoch)
    print("Loading model: ", model_path)
    model.load_models(model_path)
    print("Model %d loaded" % epoch)
...
def make_env_attrs_1(env, embs, n, input_dim=input_dim):
...
DATA_DIR = 'data/'
g_path = os.path.join(DATA_DIR, network_type, dataset, 'edges.txt')

G = nx.read_edgelist(g_path, nodetype=int, data=(('weight', float),), create_using=nx.DiGraph)
print(g_path)
...
epochs = sorted([int(i[len(model_name):-4]) for i in os.listdir('models') if i.startswith(model_name)])
REPEAT_ITER = 5
for epoch in epochs:
    for n_iter in range(REPEAT_ITER):
        output_dir = os.path.join(DATA_DIR, network_type, dataset, "G-DQN", model_name, 'epoch', str(epoch))
        if os.path.exists(os.path.join(output_dir, "budget%d_n_iter_%d.txt" % (budget, n_iter))):
            print(os.path.join(output_dir, "budget%d_n_iter_%d.txt" % (budget, n_iter)), ' exists, continue')
            continue

        random_seed = 10 + n_iter
        random.seed(random_seed)
        rg = np.random.RandomState(random_seed)
        rg1 = np.random.RandomState(random_seed)
        ...
        with torch.no_grad():
            ...
            load_checkpoint(acmodel, model_name, epoch)
            acmodel.eval()
            ...
            for stps in range(budget):
                ...
                actual_action, q = get_action_curr1(s, s_embs, l)  # q1 or q2?
        ...
        with open(os.path.join(output_dir, "seed%d_n_iter_%d.txt" % (budget, n_iter)), 'w') as f:
            for s in res:
                f.write("%d\n" % s)

        with open(os.path.join(output_dir, "time_budget%d_n_iter_%d.txt" % (budget, n_iter)), 'w') as f:
            f.write("%f" % (end_time - start_time))

        with open(os.path.join(output_dir, "mem_budget%d_n_iter_%d.txt" % (budget, n_iter)), 'w') as f:
            f.write("%f" % (peak / 10 ** 6))
