...
def arg_parse():
    ...
    parser.add_argument('--model_prefix', dest='model_prefix', type=str, required=True,
                    help='Prefix of name of saved model')
    parser.add_argument('--sample_size', dest='sample_size', type=int, default=100,
                        help='size of training synthetic graphs sample')
...
# unanotate the following lines of code to train the model on synthetic graphs
# ssm_graph_path = os.path.join("train_data", "rt", f"rand_ssm_{args.sample_size}_sample.pkl")
# ssm_graphs = pickle.load(open(ssm_graph_path, 'rb'))
# graphs.extend(ssm_graphs)
# logging.info(f"{len(ssm_graphs)} SSM Random graphs loaded.")
# logging.info("calculating opts for random graphs")
# for g in tqdm(ssm_graphs):
#     opt_obj, local_obj, S_opt = influence(g, g)
#     opts.append(opt_obj)
...
node_attrs = []
for g in graphs:
    node_attrs.append(make_const_attrs(g,input_dim))
...
try:
    ...
    for ep in range(start_ep, start_ep + NUM_EP):
        ...
        for stps in range(budget):
            ...
            if rg1.rand() > args.epsilon and (replay.size > batch_size or ep == start_ep):
            ...
            if (ep == start_ep and stps < 2) or replay.size>batch_size:
        ...
        if ep%save_every == 0:
            acmodel.save_models(args.model_prefix+str(ep))
