...
class Runner:
    ...
    def evaluate(self, num_episodes=20):
        ...
        mode = 'test'
        g_names = []
        rg = (self.args.graph_nbr_train, self.args.graph_nbr_train+self.args.graph_nbr_test)
        for g_index in range(*rg):
            ...
            if self.args.mode == 'test': num_episodes = 1
                ...
                for episode in range(num_episodes):
                    ...
                    for i in range(1, self.environment.T+1):
                        ...
                        if i % self.environment.budget == 0:
                            present = self.environment.transition(pri_action)
                            presents += present
                            invited += pri_action
                            pri_action=[ ]
                        ...
                        if done:
                            ...
                            if self.args.mode == 'test':
                                if self.args.is_real_graph:
                                    output = self.results_path
                                else:
                                    output = self.results_path % g_index
                                os.makedirs(output, exist_ok=True)
                                end_time = time.time()
                                current, peak = tracemalloc.get_traced_memory()
                                with open(output + '/time_budget%d.txt' % self.environment.budget, 'w') as f:
                                    f.write("%f" %(end_time - start_time))
                                with open(output + '/seed%d.txt' % self.environment.budget, 'w') as f:
                                    for n in presents:
                                        f.write("%d\n" % n)
                                with open(output + '/memory_budget%d.txt' % self.environment.budget, 'w') as f:
                                    f.write(f"{peak / 10**6}")
