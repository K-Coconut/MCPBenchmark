...
def load_grah(args):
    ...
    if args.mode == 'train':
        ...
        # skip evaluation during training to improve training efficiency.
        return graph_dic
    ...
    if args.is_real_graph:
        for i, graph_ in enumerate(range(args.graph_nbr_train, args.graph_nbr_train+args.graph_nbr_test)):
            G = nx.read_edgelist(os.path.join(path, 'data', args.real_graph_name, 'validation', 'sub%d' % (i + 1), 'edges.txt'), nodetype=int, data=(("weight", float),) ,create_using=nx.DiGraph)
            seed = 100000 + i if args.mode == 'train' else 200000 + i #if test then use another seed
            graph_dic[graph_] = Graph(g=G, args=args, seed=seed, is_train=is_train)
            graph_dic[graph_].graph_name = str(seed)
    else:
        print('Loading validation/test graph: ', args.graph_type)
        for i, graph_ in enumerate(range(10, 20)): # using wt mode dataset to test
            seed = 100000 + i if args.mode == 'train' else 200000 + i #if test then use another seed
            G = nx.read_edgelist(os.path.join(path, 'data', 'synthetic', args.graph_type, 'G%d' % graph_, 'edges.txt'),
                                 nodetype=int, data=(("weight", float),), create_using=nx.DiGraph)
            graph_dic[graph_] = Graph(g=G, graph_type=args.graph_type, cur_n=args.node_test, p=args.p, m=args.m, seed=seed, args=args, is_train=is_train)
            graph_dic[graph_].graph_name = str(seed)
