...
class DQAgent:
    ...
    def reset(self, g_index, mode='train'):
        ...
        self.adj = self.graphs[self.games].sp_adj
        self.edges_weight_adj = self.graphs[self.games].edges_weight_adj
    ...
    def act(self, observation, feasible_actions, mode, mask=None):
        ...
        if self.args.use_cuda:
            ...
            self.edges_weight_adj = self.edges_weight_adj.cuda()
        ...
        if self.curr_epsilon > np.random.rand() and mode != 'test':
            ...
            q_a = self.model(observation, self.adj, self.edges_weight_adj, mask=mask)
    ...
    def reward(self, observation, action, reward, done, mask):
        ...
        if len(self.memory_n) > self.minibatch_length + self.n_step:
            ...
            (last_observation_tens, action_tens, reward_tens, observation_tens, done_tens, adj_tens, edge_weight_adj_tens, obs_mask) = self.get_sample()
            if self.args.model_scheme == 'type2':
                ...
                for last_obs, act, reward, obs, done, adj, mask in zip(last_observation_tens, action_tens, reward_tens, observation_tens, done_tens, adj_tens, obs_mask):
                    aux_tensor = self.to_cuda(obs * (-1e5))
                    q = self.model(self.to_cuda(obs), self.to_cuda(th.from_numpy(adj)), self.to_cuda(edge_weight_adj_tens), mask=self.to_cuda(mask))
                ...
                aux_tensor = self.to_cuda(observation_tens * (-1e5))
                q = self.model(self.to_cuda(observation_tens) + aux_tensor, self.to_cuda(adj_tens), self.to_cuda(edge_weight_adj_tens), mask=self.to_cuda(obs_mask))
                target = self.to_cuda(reward_tens) + self.gamma * (1-self.to_cuda(done_tens)) * \
                    torch.max(q, dim=1)[0]
                target_f = self.model(self.to_cuda(last_observation_tens), self.to_cuda(adj_tens), self.to_cuda(edge_weight_adj_tens), mask=self.to_cuda(obs_mask))
                target_p = target_f.clone()
    ...
    def get_sample(self):
        ...
        edges_weight_adj_tens = self.graphs[minibatch[0][5]].edges_weight_adj.unsqueeze(0).to_dense()
        if self.args.model_scheme == 'type1':
            adj_tens = self._pad_adj(self.graphs[minibatch[0][5]], adj_tens)
            edges_weight_adj_tens = self._pad_adj(self.graphs[minibatch[0][5]], edges_weight_adj_tens)
        ...
        for last_observation_, action_, reward_, observation_, done_, games_, mask_ in minibatch[-self.minibatch_length + 1:]:
            ...
            edges_weight_adj = self.graphs[games_].edges_weight_adj.unsqueeze(0).to_dense()
            if self.args.model_scheme == 'type1':
                adj_ = self._pad_adj(self.graphs[games_], adj_).numpy()
                edges_weight_adj = self._pad_adj(self.graphs[games_], edges_weight_adj)

            edges_weight_adj_tens = torch.cat((edges_weight_adj_tens, edges_weight_adj))
            adj = torch.from_numpy(np.expand_dims(adj_.astype(int), axis=0)).type(torch.FloatTensor)
            adj_tens = torch.cat((adj_tens, adj))
        return (last_observation_tens, action_tens, reward_tens, observation_tens,done_tens, adj_tens, edges_weight_adj_tens, obs_mask)
