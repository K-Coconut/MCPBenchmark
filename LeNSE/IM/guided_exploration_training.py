...
if __name__ == "__main__":    
    ...
    logger = utils.get_logger("guided_exploration_training", os.path.join("log", f"guided_train_{opt.dataset}_{opt.weight_model}.log"))
    logger.info("=" * 150)
    logger.info(opt)
    ...
    if train_size_proportion != 20:
        graph_name = os.path.join(
            opt.dataset, opt.weight_model, "train_size_exp", f"train_size_{train_size_proportion}", "train"
        )
    else:
        graph_name = os.path.join(opt.dataset, opt.weight_model, "train")
    ...
    ckpt_path = os.path.join(base_dir, f"budget_{soln_budget}", encoder_name, "ckpt")
    ...
    graph = nx.read_edgelist(os.path.join(base_dir, "edges.txt"), nodetype=int, data=(('weight', float),), create_using=nx.DiGraph())
    ...
    for episode in range(num_eps):
        ...
        state = env.reset() # shape: embedding size + node_feat * 2
        done = False
        count = 0
        while not done:
            count += 1
        ...
        if (episode + 1) % 1 == 0:
            ...
            with open(f"{ckpt_path}/trained_dqn_{episode}", mode="wb") as f:
    ...
    dqn_.net = dqn.net
    with open(f"{ckpt_path}/trained_dqn_{num_eps}", mode="wb") as f:
        pickle.dump(dqn_, f)
