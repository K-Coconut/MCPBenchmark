...
from functions import relabel_graph, make_graph_features_for_encoder, relabel_edgelist, greedy_mvc, lazy_adaptive_greedy, calculate_cover
import random
import torch
...
class BaseEnvironment:
    ...
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, action_limit=5000, device="cpu", training=True):
    ...
    def get_optimal_scores(self):
        ...
        try:
            ...
            seeds = lazy_adaptive_greedy(self.graph, self.solution_budget)
            score = calculate_cover(self.graph, seeds)
            self.optimal_score = score

    def run_mvc_on_final_subgraph(self, budget):
        graph, transformation = relabel_graph(self.state, True)
        seeds = lazy_adaptive_greedy(graph, budget)
...
class GraphModification(BaseEnvironment):
    def __init__(self, graph, solution_budget, subgraph_size, encoder, best_embedding, graph_name, action_limit=5000, beta=1, device="cpu", training=True):

        super(GraphModification, self).__init__(graph, solution_budget, subgraph_size, encoder, graph_name, action_limit, device=device, training=training)
...
class VisualisationEnvironment(GraphModification):
    def __init__(self, graph, solution_budget, subgraph_size, encoder, best_embedding, graph_name, action_limit=100, device="cpu"):

        super(VisualisationEnvironment, self).__init__(graph, solution_budget, subgraph_size, encoder, best_embedding, graph_name, action_limit, 1, device)
...
class TestEnv(GraphModification):
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, action_limit=1000, beta=1, device="cpu"):

        super(TestEnv, self).__init__(graph, solution_budget, subgraph_size, encoder, None, graph_name, action_limit, beta, device, training=False)
    ...
    def calculate_cover_on_final_subgraph(self, budget):
        h_start = time.time()
        seeds = self.run_mvc_on_final_subgraph(budget)
        heuristic_time = time.time() - h_start
        coverage = calculate_cover(self.graph, seeds)
        return coverage, heuristic_time, seeds


class TestEnvMultiBudget(TestEnv):

    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, action_limit=1000, beta=1, device="cpu", budgets=None):
        ...
        self.optimal_scores = {}
        super(TestEnvMultiBudget, self).__init__(graph, solution_budget, subgraph_size, encoder, graph_name, action_limit, beta, device)
        self.ratios = {}
        self.times = {}
    ...
    def get_optimal_scores(self):
        for budget in self.budgets:
            try:
                ...
                seeds = lazy_adaptive_greedy(self.graph, budget)
                score = calculate_cover(self.graph, seeds)
...
class BigGraphMultiBudget(TestEnvMultiBudget):
    ...
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, action_limit=1000, device="cpu", budgets=None):

        super(BigGraphMultiBudget, self).__init__(graph, solution_budget, subgraph_size, encoder, graph_name, action_limit, 1, device, budgets)
...
class BigGraph(TestEnv):
    ...
    def __init__(self, graph, solution_budget, subgraph_size, encoder, graph_name, action_limit=1000, device="cpu"):

        super(BigGraph, self).__init__(graph, solution_budget, subgraph_size, encoder, graph_name, action_limit, 1, device)
