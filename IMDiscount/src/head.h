...
static inline void trim(std::string &s) {rtrim(s);}
...
string currentTimestampStr() {
    ...
    char buf[1000];
    sprintf(buf, "%04d-%0s2d-%02d %02d:%02d:%02d", now->tm_year+1900, now->tm_mon+1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec);
    return string(buf);
}
